 ...
 *
 */
public class IsGreenImageUDF implements UDF {
	public static final String ID_STRING = "is_green_image";
	String udfDesc;

	public static UDF createUDF(String udfDesc) {
		return new IsGreenImageUDF(udfDesc);
	}

	public IsGreenImageUDF(String udfDesc) {
	...
	public static double getGreenPct(BufferedImage img) {
		int greenCnt = 0;
		...
		for (int w = 0; w < img.getWidth(); w++) {
			for (int h = 0; h < img.getHeight(); h++) {
				...
				if (green > 50 && green > red && green > blue && red < 0.98 * green && blue < 0.98 * green && 
						((double) red/blue > 0.667 && (double) red/blue < 1.5)) {
					greenCnt++;
				}
			}
		}

		return (double) greenCnt/numPixels;
	}
	...
	public static void main(String[] args) {
		String fname = "test_data/Caltech101/ibis/image_0007.jpg";
		System.out.println(fname);

		BufferedImage img = null;
		try {
			img = ImageIO.read(new File(fname));
			System.out.println(IsGreenImageUDF.getGreenPct(img));
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
