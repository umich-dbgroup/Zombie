 ...
 *
 */
public class IsRedImageUDF implements UDF {
	public static final String ID_STRING = "is_red_image";
	String udfDesc;

	public static UDF createUDF(String udfDesc) {
		return new IsRedImageUDF(udfDesc);
	}

	public IsRedImageUDF(String udfDesc) {
	...
	public static double getRedPct(BufferedImage img) {
		int redCnt = 0;
		...
		for (int w = 0; w < img.getWidth(); w++) {
			for (int h = 0; h < img.getHeight(); h++) {
				...
				if (red > 50 && red > blue && red > green && blue < 0.98 * red && green < 0.98 * red && 
						((double) blue/green > 0.667 && (double) blue/green < 1.5)) {
					redCnt++;
				}
			}
		}

		return (double) redCnt/numPixels;
	}
	...
	public static void main(String[] args) {
		...
		try {
			...
			System.out.println(IsRedImageUDF.getRedPct(img));
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
