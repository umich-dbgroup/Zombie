 ...
 *
 */
public class LinearRegressionTask implements LearningTask {
	public static final String ID_STRING = "linear_regression";
	...
	LinearRegression cModel;

	double performance = 0.0;
	private double pctCorrect;
	int stepsUntilRetrain = 100;
	...
	public LinearRegressionTask(String taskDesc) {
	...
	public LinearRegressionTask(String taskDesc, DataSource data, ArrayList<UDF> udfs) {
		this.data = data;
	...
	public void provideData(Writable udfInputKey, Writable udfInputVal,
		...
		try {
			...
			Item item = data.getItemCache().get(itemId);
			...
			if (featVals.size() == featureCount) {
				...
				variance += Double.valueOf(itemLabel)*Double.valueOf(itemLabel);
				
				//System.out.println("STD: " + Math.sqrt(variance/(itemIds.size() + 1)));
				
				labelTime = System.nanoTime() - startLabelTime;

				boolean isGoodLabel = true;
				//				int labelId = 0;
				//				int i = 0;
				//				for (String lbl : labels) {
				...
				if (isGoodLabel) {			
					...
					if (wekaTrain.size() == 0 || cModel == null) {
						...
						System.out.println("size: " + wekaTrain.size() + " isnull: " + (cModel==null));
					}
					else {
						performance = 1;
						stepsUntilRetrain--;
						...
						predictedValue = eTest.evaluateModelOnce(cModel, itemInstance);
					...
					if (wekaTrain.size() > 0) {
						...
						if (stepsUntilRetrain == 0) {
							...
							pctCorrect = eTest.rootMeanSquaredError();
						...
						if (rewardType.equals("roundRobin")) {
							performance = 0.0;
						}
						...
						else if (rewardType.equals("cosineDistance")) {
							performance = performanceCosineDistance(featVals);
						}
						else if (rewardType.equals("wronglyClassified")) {
							performance = performanceWronglyClassified();
						}
						else if (rewardType.equals("infoTrace")) {
							performance = performanceInfoTrace(featVals);
						}
						else if (rewardType.equals("eucDist")) {
							performance = performanceEucDist(featVals);
						}
						else {
	...
	private void buildTestSet() {
		...
		String arffFileName = "wekaTestLinReg.arff";
		if (ExperimentParameters.check("numClasses", "2"))
			arffFileName = "wekaTestLinReg.arff.2class";
		else if (ExperimentParameters.check("numClasses", "6"))
			arffFileName = "wekaTestLinReg.arff.6class";
		...
		for (Item item : testItems) {
			...
			if (!wekaTestLoaded) {
				...
				try {
					for (UDF udf : udfList) {
						GenericRecord udfOutput = udf.createOutput(new IntWritable(item.getItemId()), new Text(data.getDocumentContents(item.getItemId())));
					...
					if (item.getLabel() != null)
						inst.setClassValue(Double.valueOf(item.getLabel()));
					else
						inst.setClassValue(Double.valueOf(labeler.getLabel(id)));
	...
	private Instance createInstance(int docId, ArrayList<Double> featVals) {
		...
		Item item = data.getItemCache().get(docId);
		String lbl = null;
		if (item.getLabel() != null)
			lbl = item.getLabel();
		else
			lbl = labeler.getLabel(docId);

		Double cls = Double.valueOf(lbl);
		...
		if (inst != null) {
			...
			if (retrain) {
				...
				try {
					...
					//					} else {
					cModel = new LinearRegression();
					cModel.setOptions(weka.core.Utils.splitOptions("-S 1 -C"));
	...
	private double performanceWronglyClassified() {
		...
		perf = Math.abs(Double.valueOf(itemLabel) - predictedValue);

		//return 1.0 - 4*perf /(1 + 4*perf);
		return 1/perf;
	}

	private double performanceInfoTrace(ArrayList<Double> featureValues) {
		double perf = 0.0;
	...
	private double performanceEucDist(ArrayList<Double> featureValues) {
		double perf = 0.0;
		...
		for (int i = 0; i < featureValues.size(); i++) {
			double val = featureValues.get(i) - centroid.get(i)/wekaTrain.size();
			perf += val * val;
			centroid.set(i, centroid.get(i) + featureValues.get(i));
		}
			
		return Math.sqrt(perf);
	}
	
	private double performanceCosineDistance(ArrayList<Double> featureValues) {
		double perf = 0.0;

		if (wekaTrain.numInstances() > 0) {
			double[][] featArray = new double[1][featureValues.size() + 1];
			for (int i = 0; i < featureValues.size(); i++) {
				featArray[0][i] = featureValues.get(i);
			}
			featArray[0][featureValues.size()] = 1.0;

			SimpleMatrix features = new SimpleMatrix(featArray);
			SimpleMatrix trainingSet = new SimpleMatrix(wekaTrain.numInstances(),featureValues.size() + 1);

			for (int i = 0; i < trainingSet.numRows(); i++) {
				double[] inst = wekaTrain.instance(i).toDoubleArray();
				for (int j = 0; j < inst.length - 1; j++) { // -1, the last element is the label
					trainingSet.set(i, j, inst[j]);
				}
				trainingSet.set(i, inst.length-1, 1.0);
			}


//			SimpleMatrix smoother = new SimpleMatrix(wekaTrain.numInstances(),featureValues.size());
//			smoother.set(0.1);
//
//			trainingSet = trainingSet.plus(smoother);

			SimpleMatrix newTrainingSet = trainingSet.combine(SimpleMatrix.END, 0, features);

			double oldInfo = trainingSet.transpose().mult(trainingSet).trace();
			double newInfo = newTrainingSet.transpose().mult(newTrainingSet).trace();
			perf = newInfo - oldInfo;
			perf = perf/oldInfo;
			
//			if (perf/oldInfo > 0.0005) perf = 1;
//			else perf = 0;

			//			double featNorm = features.normF();
			//			double totalCosDist = 0;
			//			
			//			for (int i = 0; i < trainingSet.numRows(); i++) {
			//				SimpleMatrix example = trainingSet.extractVector(true, i);
			//				totalCosDist += features.dot(example)/(featNorm * example.normF()); 
			//			}
			//			
			//			perf = 1.0 - totalCosDist/trainingSet.numRows();
			//			if (perf > 0.5) perf = 1.0;
			//			else perf = 0.0;
